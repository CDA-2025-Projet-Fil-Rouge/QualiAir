...
public class FavorisServiceImpl implements FavorisService {
    ...
    public List<InfoFavorite> getAllUserFavorites(CusomUserPrincipal user) {
        ...
        return communes.stream().map(c -> mapper.toFavoritesView(c, userId)).toList();
    }

    @Transactional
    @Override
    public List<InfoFavorite> addNewUserFavorite(CusomUserPrincipal user, CodeInsee codeInsee) throws DataNotFoundException, UpdateException, BusinessRuleException {
        Utilisateur utilisateur = utilisateurService.getUserById(user.getId());
        String code = codeInsee.getCodeInsee();
        Commune commune = communeService.getCommuneByCodeInsee(code);
        ...
        return utilisateur.getFavCommunes().stream().map(c -> mapper.toFavoritesView(c, user.getId())).toList();
    }

    @Transactional
    @Override
    public List<InfoFavorite> removeUserFavorite(CusomUserPrincipal user, String codeInsee) throws DataNotFoundException, BusinessRuleException {
        Utilisateur utilisateur = utilisateurService.getUserById(user.getId());
        Commune commune = communeService.getCommuneByCodeInsee(codeInsee);
        if (!utilisateur.getFavCommunes().contains(commune)) {
