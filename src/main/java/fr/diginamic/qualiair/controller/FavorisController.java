...
public interface FavorisController {
     ...
     *
     * @param user      L'utilisateur authentifié via Spring Security.
     * @param codeInsee L'identifiant de la commune à ajouter aux favoris.
     ...
     */
    @PostMapping("/add")
    ResponseEntity<List<InfoFavorite>> addNew(@AuthenticationPrincipal CusomUserPrincipal user, @RequestBody CodeInsee codeInsee) throws DataNotFoundException, BusinessRuleException, UpdateException;

    /**
     * Supprime une commune des favoris de l'utilisateur connecté.
     *
     * @param user      L'utilisateur authentifié via Spring Security.
     * @param codeInsee L'identifiant de la commune à retirer des favoris.
     * @return Une réponse HTTP contenant la liste mise à jour des favoris.
    ...
    ResponseEntity<List<InfoFavorite>> removeById(@AuthenticationPrincipal CusomUserPrincipal user, @PathVariable String codeInsee) throws DataNotFoundException, BusinessRuleException;
}
